...
| Groove lags (ride +8, snare +12, kick -3, hi-hat -4 ms), drift sigma 5 ms / correlation 0.9, phrase length 4 bars, motif shadow probability 0.35 + 0.25 x energy | see `arrange/interplay.py` | Assumption | bass -5 ms and piano +10 ms are consistent with the Jazz Trio Database medians (data resolution 10 ms); the rest are hand-set feel parameters, tune by ear |
| Horn attack compensation | sax notes start 28 ms early | Measured (one library) | rendered tenor-sax SFZ (FreePats): the level reaches half its note level a median 28 ms (quartiles 18 / 41, 18 clean onsets) after the note-on; other sax libraries will differ |
| Horn minimum note 0.11 s (shorter notes are lengthened, under 0.06 s dropped), no separate grace notes for horns | | Assumption | sample-library saxophones click or break on very short notes; the measured case had 44 notes under 80 ms of 464 |
| Swaralipi: swara -> semitone (S 0, r 1, R 2, g 3, G 4, m 5, M 6, P 7, d 8, D 9, n 10, N 11), one swara = one matra, taal cycles (dadra 6, kaharwa 8, tritaal 16, ektaal 12) | see `analysis/swaralipi.py` | Published (convention) | standard sargam degrees and taal cycle lengths; the mapping of a cycle onto the arranger's 2/3/4/6 meters is a fold chosen by the tool, and the written bar marks override it |
| Melody cleaning: minimum note 0.13 s, merge gap 0.09 s, out-of-scale glide under 0.4 s, pitch class rarer than 5% of the song | see `arrange/melodyline.py` | Assumption | chosen so the cleaned line matches what a listener hums; validated only by the note statistics (496 -> 342 notes, median 0.16 -> 0.30 s) and by review, not against a hand-transcribed melody |
| Voicing under a melody: shells preferred, minor ninth cost 1.0 and the reverse semitone 0.45, melody notes under 25% of a chord's span ignored, melody-fit weight 2.2 in the chord solver | see `arrange/theory.py`, `arranger.solve_chords` | Assumption (standard practice) | jazz voicing rules avoid minor ninths between voices and pianists play shells behind a singer; the numbers are hand-set and were tuned until the measured clash rate fell (46% -> 11%) |
